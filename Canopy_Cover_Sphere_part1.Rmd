...
##### Notrun
all_pic_paths <- list.files("All_Photos", full.names = TRUE)


all_light_indices <- get_indicies_from_paths(all_pic_paths[1])[0,]

counter = 1
starttime <-paste(Sys.time())
for (pic in all_pic_paths){
  
  print(paste0(nrow(all_light_indices), "/", length(all_pic_paths),  ", started at ", starttime))
  
  m_fe <- import_fisheye(pic,
                         display = FALSE,
                         message = F)
  
  b_fe <- binarize_fisheye(m_fe,
                           display = FALSE)
  
  gap_fe <-  gapfrac_fisheye(b_fe,
                             lens = "orthographic",
                             display = FALSE,
                             message = F,
                             nseg = 1,
  ...
  df <- canopy_fisheye(gap_fe)
  
  df <- get_indicies_from_paths(pic)
  all_light_indices <- rows_append(all_light_indices, df)
  counter = counter + 1
  
  if (nrow(all_light_indices) == length(all_pic_paths)){
    
    write.csv2(all_light_indices, file = "all_photos.csv")
    
    break
  }
}
  
#all_light_indices <- get_indicies_from_paths(all_pic_paths, message=TRUE)  # too big (not enough memory)

all_light_indices <- all_light_indices[ , c(1:3,7:8, 11:12)]
colnames(all_light_indices) <- c("Pic_Name", "eff_LAI", "act_LAI","Gap_Frac", "Mean_Leaf_Angle", "Rings", "Azimuths")
write.csv2(all_light_indices, file = "all_photos_compressed.csv")
```

then we just check the first 10 rows to see what we get:

```{r check light indicies, echo=FALSE}
